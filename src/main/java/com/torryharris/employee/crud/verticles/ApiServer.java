...
public class ApiServer extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    router.post("/employees").consumes("*/json").handler(BodyHandler.create()).produces("application/json")
      .handler(routingContext -> {
        ...
        JsonObject json = new JsonObject()
          .put("message", "Employee records added");
    ...
    router.delete("/employees/:id").produces("*/json")
      .handler(routingContext -> {
        String id = routingContext.request().getParam("id");
        employeeController.deleteEmployee(id).future()
          .onSuccess(response -> sendResponse(routingContext, response));
        System.out.println(" employee records deleted");
        JsonObject json = new JsonObject()
          .put("message", "Employee record deleted");
        routingContext.response()
          .putHeader("content-type", "application/json; charset=UTF8")
          .end(json.encodePrettily());
      });
    ...
    router.put("/employees").consumes("*/json").handler(BodyHandler.create()).produces("*/json")
      .handler(routingContext -> {
        Employee employee = Json.decodeValue(routingContext.getBody(), Employee.class);
        employeeController.putEmployee(employee).future()
          .onSuccess(response -> sendResponse(routingContext, response));
        JsonObject json = new JsonObject()
          .put("message", "Employee records updated");
        routingContext.response()
          .putHeader("content-type", "application/json; charset=UTF8")
          .end(json.encodePrettily());
        System.out.println(routingContext.getBodyAsJson());
